    ...
    navigator.mediaDevices
      ...
      .then(async function (stream) {
        ...
        await audioContext.audioWorklet.addModule(
          chrome.runtime.getURL("pitch.detector.audioworklet.js"),
        );

        console.log("Loaded audio worklet module...");

        audioWorkerNode = new AudioWorkletNode(
          audioContext,
          "pitch-detector-audio-processor",
          {
            numberOfOutputs: 0,
            numberOfInputs: 1,
            processorOptions: {
...
const transpose = 20; //0-50; //cents
var follower;
var currentX = 10;
var currentY = 10;
const cursor_movement = 3;
var lastClickMillis = 0;
...
function createCursorFollower() {
  ...
  if (document.body && document.body.style) document.body.style.cursor = "none";
...
function handlePitchEstimates(estimation) {
  ...
  switch (midi_pitch_class) {
    case 11:
      ...
      let element = document.elementFromPoint(currentX, currentY);
      if (element != null) {
        const currentClickMillis = new Date().getTime();
        //debounce, we do not want more than one click per 500ms
        const delta = currentClickMillis - lastClickMillis;
        if (delta > 500) {
          console.log("Pitch clicking");
